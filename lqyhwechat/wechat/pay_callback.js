...
let PayCallBack = (req, res, next) => {
    ...
    if (jsonData.result_code == 'SUCCESS') {
        ...
        if (global.total_fee == jsonData.total_fee) {
            // JSON.parse(jsonData.attach)
            let userid = JSON.parse(jsonData.attach).userid
            let actionname = '支付完成'
            let actiondesc = '支付完成'
            let actionseq = 2
            let time = sd.format(new Date(), 'YYYY-MM-DD HH:mm')
            let str = ''
            let amount = jsonData.total_fee
            JSON.parse(jsonData.attach).orderid.split(',').map((item, index) => {
                if (index == 0) {
            ...
            let sql1 = `update order_info set status=1,updatetime='${time}' where id in '(${JSON.parse(jsonData.attach).orderid})'`
